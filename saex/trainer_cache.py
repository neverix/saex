...
class ActivationBuffer(eqx.Module):
    ...
    def digest(self, activations, state, mask=None):
        ...
        new_cache = jax.ops.index_update(cache, indices, activations)
        state = state.set(self._cache, new_cache)
        state = state.set(self._n_valid, new_n_valid)
        return state
...
class BufferTrainer(object):
    def __init__(self, config: BufferTrainerConfig, model=None, dataset=None):
        self.config = config
        ...
        self.model = model
        if dataset is None:
            dataset = create_iterable_dataset(config.dataset_config)
    ...
    def train(self):
        ...
        for iteration in bar:
            ...
            activations, model_misc = self.model.digest(texts)
            cache_kwargs = {k: v for k, v in model_misc.items()
                            if k in ["mask"]}
            self.buffer_state = self.buffer.digest(activations, self.buffer_state, **cache_kwargs)
